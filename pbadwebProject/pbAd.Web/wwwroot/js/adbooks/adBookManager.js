...
var adBookManager = {
    init: function () {
        /*
        //======Test Start===========================
        var addContentText = `নৌপরিবহন মন্ত্রণালয়ের অধীন নৌপরিবহন অধিদপ্তর শূন্য পদে জনবল নিয়োগের বিজ্ঞপ্তি প্রকাশ করেছে। নৌপরিবহন অধিদপ্তর সম্প্রতি ৫টি পদে ১৯ জনকে নিয়োগের জন্য এ বিজ্ঞপ্তি প্রকাশ করা হয়েছে। বিজ্ঞপ্তি অনুসারে`;
        $('textarea[name="AdContent"]').val(addContentText);
        $('#previousAdCompose').val(addContentText);
        //======Test End============================
        */
    ...
    getDiscountByDateOffer: function () {
        ...
        $.ajax({
            ...
            error: function (err) {
                ...
                app.Notify(AlertTypeConstants.Error, msg, AlertIconConstants.Error);
            }
        })
    },
    getTotalWords: function () {
        ...
        if (!$('textarea[name="AdContent"]') || $('textarea[name="AdContent"]').length <= 0) {
            ...
            adBookManager.getEstimatedCosting();
            return;
        }
        var totalWords = 0;
    ...
    configureAdEnhancement: function () {
        ...
        if (adEnhancementType === AddEnhancementTypeConstants.BoldInScreen) {
            $('.ad-preview').css({ 'font-weight': 'bold', 'background-color': 'rgb(214 214 214)', 'padding': '5px 10px' });
        ...
        if (adEnhancementType === AddEnhancementTypeConstants.BoldInScreenAndSingleBox) {
            $('.ad-preview').css({ 'font-weight': 'bold', 'background-color': 'rgb(214 214 214)', 'border': '2px solid black', 'padding': '5px 10px' });
        ...
        if (adEnhancementType === AddEnhancementTypeConstants.BoldInScreenAndDoubleBoxes) {
            $('.ad-preview').css({ 'font-weight': 'bold', 'background-color': 'rgb(214 214 214)', 'border': '2px solid black' });
            var appendedHtml = $('.ad-preview').html();
            $('.ad-preview').html(`<div style='border: 2px solid black; margin:1px;padding:5px 10px;'> ${appendedHtml} </div>`);
    ...
    addDateOffer: function (dateBasedOffer) {
        ...
        if (isExistInDatesBasedOffer) {
            app.Notify(AlertTypeConstants.Warning, `Warning! Date already selected`, AlertIconConstants.Warning);
            return;
        }
        ...
        adBookManager.generateBaseOfferDateContainerHtml();

        //get discount by date offer          
        adBookManager.getDiscountByDateOffer();
...
$(function () {
    ...
    $('#CategoryId').on('change', function () {
        ...
        adBookManager.loadSubCategoryByCategory(categoryId);
    })
    ...
    $('textarea[name="AdContent"]').on('keyup', function (e) {
        var totalWords = adBookManager.totalWords();
        var maxWords = parseInt($('#hiddenMaxWords').val());
        if ((e.keyCode !== 8) && totalWords > maxWords) {
        ...
        adBookManager.configureAdEnhancement();
    ...
    $('.btn-date-based-offer').on('click', function () {
        ...
        if (!(tomorrowFormattedDate === dateBasedOffer)) { //check tomorrow booking
            adBookManager.addDateOffer(dateBasedOffer);
            return;
        }
    ...
    $("#AgencyAutoComplete").autocomplete({
        ...
        select: function (event, ui) {
            ...
            if (ui.item.value > 0) adBookManager.togglePersonal();
