...
var bookDigitalDisplayAdManager = {
    init: function () {
        ...
        $("input[name=ScreenType][value=" + ScreenTypeConstants.Desktop + "]").attr('checked', 'checked');
    },
    ...
    getEstimatedCosting: function () {
        ...
        var isValidToAddNew = bookDigitalDisplayAdManager.validateNewDetails();
        ...
        if (!(digitalAdUnitTypeId > 0 && digitalPageId > 0 && digitalPagePositionId > 0)) {
            bookDigitalDisplayAdManager.populateDigitalDisplayDetail(0);
            return;
        }
        ...
        $.ajax({
            ...
            success: function (data) {
                if (!data) {
                    ...
                    $('#EstimatedTotal').val(estimatedCosting);
                    */
                    bookDigitalDisplayAdManager.populateDigitalDisplayDetail(0);
                    return;
                }
                ...
                bookDigitalDisplayAdManager.populateDigitalDisplayDetail(parseInt(estimatedCosting)) 
                //$('.esitmated-cost-amount').text(estimatedCosting);
                //$('#EstimatedTotal').val(estimatedCosting);
            },
            error: function (request, status, error) {
        ...
        $.ajax({
            ...
            success: function (data) {
                if (!data) {
                    ...
                    bookDigitalDisplayAdManager.initAutoComForAdvertiser();
...
$(function () {
    bookDigitalDisplayAdManager.init();


    $('.btn-gd-detail').on('click', function () {
        bookDigitalDisplayAdManager.getEstimatedCosting();        
    })

    $('#tbleDigitalDisplayDetail').on('click','.btn-remove-gd-detail', function () {
        var serialNo = $(this).attr('serialNo');
        //remove gd detail
        bookDigitalDisplayAdManager.removeGdDetail(serialNo);
    ...
    $('#CategoryId').on('change', function () {
        ...
        bookDigitalDisplayAdManager.loadSubCategoryByCategory(categoryId);
    })
    ...
    $('#DigitalPageId').on('change', function () {
        ...
        bookDigitalDisplayAdManager.loadPagePositionByPage(digitalPageId);
    })
    ...
    $("#BrandAutoComplete").autocomplete({
        ...
        select: function (event, ui) {
            ...
            bookDigitalDisplayAdManager.loadCategory();

            //get brand wise advirtiser
            bookDigitalDisplayAdManager.GetBrandWiseAdvirtiser();
    ...
    $('.tbl-uploded-files').on('click', '.cancel-file', function () {
        ...
        bookDigitalDisplayAdManager.removeUploadedFile(fileName);
    });
