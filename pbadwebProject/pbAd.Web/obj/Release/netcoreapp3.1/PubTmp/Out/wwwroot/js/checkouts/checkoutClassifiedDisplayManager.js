...
var checkoutClassifiedDisplayManager = {
    init: function () {
        ...
        checkoutClassifiedDisplayManager.defaultNationalEditionChecked();

        // get estimated costing
        checkoutClassifiedDisplayManager.getEstimatedCosting();
    },
    ...
    getEstimatedCosting: function () {
        var masterTableId = $('#ABPrintClassifiedDisplayId').val();
        ...
        if (!masterTableId || !bookingNo) { app.Notify(AlertTypeConstants.Warning, 'Booking No. not found. Please contact support team!', AlertIconConstants.Warning); return };
        ...
        $.ajax({
            url: '/CheckoutClassifiedDisplay/GetOrderTotalAmount',
            type: "POST",
            data: formData,
            dataType: "json",
            success: function (data) {
                if (!data.status) {
                    ...
                    //get total discounts
                    checkoutClassifiedDisplayManager.getTotalDiscounts();
    ...
    getDefaultDiscount: function () {
        ...
        $.ajax({
            url: '/CheckoutClassifiedDisplay/GetManualDiscount',
            type: "POST",
            data: formData,
            dataType: "json",
            success: function (data) {
    ...
    getTotalDiscounts: function () {
        ...
        var totalDiscount = parseFloat(dateOfferDiscount) + Number(editionOfferDiscount) + Number(mannualDiscount);
        $('.total-discounts').text(parseInt(Math.round( totalDiscount)));
    },
}

$(function () {
    checkoutClassifiedDisplayManager.init();
    /*
    ...
    */
    ...
    $(':checkbox[name=EditionIds]').on('click', function () {
        ...
        if (!checkoutClassifiedDisplayManager.isNatinalEdition()) {
    ...
    $('input[type="radio"][name="PaymentModeId"]').on('click', function () {
        ...
        checkoutClassifiedDisplayManager.getDefaultDiscount();
    })
    ...
    $("#form-classified-display-checkout-payment").on("submit", function (event) {
        event.preventDefault();
        var _currentForm = $(this).closest('#form-classified-display-checkout-payment');
