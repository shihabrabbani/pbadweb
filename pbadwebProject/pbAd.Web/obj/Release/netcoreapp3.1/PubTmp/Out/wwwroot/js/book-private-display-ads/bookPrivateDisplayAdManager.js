...
var bookPrivateDisplayManager = {
        ...
        $.ajax({
            ...
            success: function (data) {
                if (!data) {                  
                    ...
                    bookPrivateDisplayManager.initAutoComForAdvertiser();
                ...
                //personal actions
                bookPrivateDisplayManager.personalActions(); 
            },
            error: function (request, status, error) {
                alert(request.statusText + "/" + request.statusText + "/" + error);
            }
    ...
    addDateOffer: function (dateBasedOffer) {
        ...
        bookPrivateDisplayManager.generateBaseOfferDateContainerHtml();

        //get discount by date offer          
        bookPrivateDisplayManager.getDiscountByDateOffer();
    ...
    initAutoComForAdvertiser: function () {
        var $advertiserName = $("#AdvertiserName"); $advertiserName.val('');
        var $advertiserId = $("#AdvertiserId"); $advertiserId.val('');
...
$(function () {
    bookPrivateDisplayManager.init();

    $('#ColumnSize').on('change', function () {
        //get estimated costing
        bookPrivateDisplayManager.getEstimatedCosting();
    })
    ...
    */
    ...
    $('.btn-date-based-offer').on('click', function () {
        ...
        if (!(tomorrowFormattedDate === dateBasedOffer)) { //check tomorrow booking
            bookPrivateDisplayManager.addDateOffer(dateBasedOffer);
            return;
        }
    ...
    $("#BrandAutoComplete").autocomplete({
        ...
        select: function (event, ui) {
            ...
            bookPrivateDisplayManager.loadCategory();

            //get brand wise advirtiser
            bookPrivateDisplayManager.GetBrandWiseAdvirtiser();
    ...
    $('.tbl-uploded-files').on('click', '.cancel-file', function () {
        ...
        bookPrivateDisplayManager.removeUploadedFile(fileName);
    });
