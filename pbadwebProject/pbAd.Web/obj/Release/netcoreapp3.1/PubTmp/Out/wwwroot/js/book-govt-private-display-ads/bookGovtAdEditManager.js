...
var bookGovtAdEditManager = {
    init: function () {
        ...
        //load category
        bookGovtAdEditManager.loadCategory(categoryId);
        //get brand wise advirtiser
        //bookGovtAdEditManager.GetBrandWiseAdvirtiser();
        ...
        bookGovtAdEditManager.getDiscountByDateOffer();
        ...
        $.ajax({
            ...
            success: function (data) {
                if (!data) {
                    ...
                    bookGovtAdEditManager.initAutoComForAdvertiser();
                ...
                //personal actions
                //bookGovtAdEditManager.personalActions();
            },
            error: function (request, status, error) {
                alert(request.statusText + "/" + request.statusText + "/" + error);
            }
    ...
    addDateOffer: function (dateBasedOffer) {
        ...
        bookGovtAdEditManager.generateBaseOfferDateContainerHtml();
...
$(function () {
    bookGovtAdEditManager.init();

    $('#ColumnSize').on('keyup blur', function () {
        //get estimated costing
        bookGovtAdEditManager.getEstimatedCosting();
    })

    $('#InchSize').on('keyup blur', function () {
        //get estimated costing
        bookGovtAdEditManager.getEstimatedCosting();
    })
    ...
    $('.btn-date-based-offer').on('click', function () {
        ...
        if (!(tomorrowFormattedDate === dateBasedOffer)) { //check tomorrow booking
            bookGovtAdEditManager.addDateOffer(dateBasedOffer);
            return;
        }
    ...
    $("#BrandAutoComplete").autocomplete({
        ...
        select: function (event, ui) {
            ...
            bookGovtAdEditManager.loadCategory();
    ...
    $('.tbl-uploded-files').on('click', '.cancel-file', function () {
        ...
        bookGovtAdEditManager.removeUploadedFile(fileName);
    });
    $('.tbl-existing-files').on('click', '.cancel-file', function () {
        ...
        //remove uploaded file
        bookGovtAdEditManager.removExistingFile(filePath);
    });
    ...
    $('#UploadLater').on('click', function () {
        //toggle upload later
        bookGovtAdEditManager.toggleUploadLater();
    })

    $("#form-edit-govt-ad").on("submit", function (event) {
        event.preventDefault();

        var _currentForm = $(this).closest('#form-edit-govt-ad');
